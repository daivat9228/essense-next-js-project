...
// Helper to get unique values from an array without using Set (avoids TS downlevel issues)
...
export default function Filters({ products }: FiltersProps) {
  ...
  // Extract unique values from products (no Set syntax now)
  const uniqueCategories = getUnique(products.map((p) => p.category));
  const uniqueFamilies = getUnique(products.map((p) => p.family));
  const uniqueConcentrations = getUnique(products.map((p) => p.concentration));
  const uniqueSizes = getUnique(
    products.flatMap((p) => p.sizes.map((s: any) => s.sizeMl as number))
  ).sort((a, b) => a - b);
  const uniqueBrands = getUnique(products.map((p) => p.brand)).sort();
  ...
  const handleFamilyChange = (family: string) => {
    const newFamilies = filters.families.includes(family)
      ? filters.families.filter((f: string) => f !== family)
  ...
  const handleConcentrationChange = (concentration: string) => {
    const newConcentrations = filters.concentrations.includes(concentration)
      ? filters.concentrations.filter((c: string) => c !== concentration)
  ...
  const handleSizeChange = (size: number) => {
    const newSizes = filters.sizes.includes(size)
      ? filters.sizes.filter((s: number) => s !== size)
      : [...filters.sizes, size];
  ...
  const handlePriceChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'min') {
      newRange[0] = parseInt(value || '0', 10);
    } else {
      newRange[1] = parseInt(value || '0', 10);
    }
